...
The original game runs on the X68000 in a resolution of 256x256 pixels. Instead of just taking the original images, I decided to use Deluxe Paint (running in an Amiga Emulator) to add additional graphics at the borders. On a PAL Genesis the demo displays graphics at a 320x240 pixels resolution. On an NTSC Genesis (320x228) some pixels at the top and bottom are cropped.
...
Converting the graphics turned out not to be as straightforward as it first looked, which is because the X68000 not only has a palette of 65536 colors, but also can display more of them simultaneously than the Genesis. To get as close as possible to the original, sprites are used as an extra layer to get around the color limits of Genesis background tiles.
